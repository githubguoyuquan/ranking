...
2. **P1 — Jobs & durability**  
   ...
   - ✅ **爬虫**：Checkpoint / `Source` / `CrawlTask` / `CrawledUrl` + BullMQ `crawl`；**`GET /v1/crawl/tasks`** 按 `limit`（1–100，默认 30）、可选 **`sourceId`** 列出近期任务（引擎监控 / 运营排障）；**可选真 HTTP**（`CRAWL_HTTP_FETCH` 或 `kind: http-fetch`）或 **Playwright**（`CRAWL_USE_PLAYWRIGHT` / `kind: http-playwright`）：`contentHash`、`textPreview`、基础 SSRF；**`CRAWL_HTTP_PROXY` / `Source.httpProxyUrl`** 出站代理；**多实例**共用 Redis 消费同一 `crawl` 队列（`npm run start:crawl-worker` 独立 Worker 进程）；**`CRAWL_PER_HOST_MIN_INTERVAL_MS`** Redis 同 host 节流；**`CRAWL_SEMANTIC_DEDUP`** 用本地确定性向量做同信源正文去重（`fetched_semantic_dup` / `duplicateOfId`，不重复入 ES）；未开真抓取时仍为桩 `fetched_stub`
3. **P2 — Analytics & search**  
   ...
   - ✅ **BI 大屏（完整聚合）**：`/bi` · `GET /admin/bi/overview`（PG + **ClickHouse** 话题趋势 + Outbox 分型 + 全球爬虫 KPI + ES/Qdrant 规模）；`GET /admin/bi/timeseries/topics`、`entities/:id/rank-sparkline`；CH MV `metric_daily_topic`  
...
4. **P3 — Agents & UI**  
   ...
   - ✅ **Agent（最小）**：`POST /admin/snapshots/:id/analyze` → `AiAnalysis`；body 可选 **`agent`（≤120）**、**`topN`（1–50）**、**`chainContext`（≤8192）**；所有摘要由项目内规则生成，不调用收费 AI。约定名 **`rules-v1`**（默认）、**`post-snapshot-summary-v1`** / **`trend-v1`** / **`credibility-v1`**；`GET /v1/snapshots/:id/analyses` 返回 **`{ filter, total, analyses }`**。
...
- 流程：**Outbox（推荐）**—— 启用 ES 时，`POST/PATCH /admin/entities` 与 `seed-demo` 在 PG 事务内写入 `OutboxEvent`（`elasticsearch.entity.sync`），进程内 Flusher 在提交后写入全文字段和项目内生成的本地向量；`DELETE /admin/entities` 先写 `delete` Outbox 再删 PG 行。算法升级后可用 `POST /admin/reindex-entities` 或 `POST /admin/reindex-crawl-docs` 全量重建索引。
...
- **`GET /v1/search`**：一次返回实体与爬取 URL；`entitySemantic=1|true` 时使用项目内 1536 维特征哈希 + ES kNN，不需要 API Key。
- **`GET /v1/search/entities`**：`engine=es|pg|auto`；`semantic=1|true` 时使用本地向量并要求 ES。
- **`GET /v1/recommendations/similar-topics?topicId=&limit=`**：基于 `TopicEmbedding`；缺失向量由项目内算法批量补齐。
...
# 同上：可选 body 如 {"agent":"trend-v1","chainContext":"…"}（≤8192，供项目内规则步骤记录上下文）
