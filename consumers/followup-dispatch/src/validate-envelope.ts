...
function getValidators() {
  if (!envelopeValidate || !payloadValidate) {
    ...
    payloadValidate = ajv.compile(
      JSON.parse(
        readFileSync(
          join(SCHEMA_DIR, 'ranking-followup-requested-payload-v1.schema.json'),
          'utf8',
        ),
      ) as object,
    );
...
export function parseRankingFollowupRequestedMessage(raw: string): ParseResult {
  let parsed: unknown;
  try {
  ...
  if (envelope.type !== OUTBOX_TYPE_RANKING_FOLLOWUP_REQUESTED) {
