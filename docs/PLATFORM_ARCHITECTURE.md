...
| 单条目排名演化 | previousRank、rankChange、趋势 | `RankingItem`：`previousRank`、`rankChange`、`TrendType`、多维度 score | 已满足；缺**历史最高/最低、连续升降天数**等派生指标持久化或聚合服务 |
| 历史时间线 | `RankingItemHistory` + 分析 | `RankingItemHistory` 存在 | 缺**按实体回溯 API**、缺 **CH 与 PG 联动报表** |
...
| 趋势分析 | 环比/同比/MA/异常 | `TrendAnalysis` 表 + `scoring` 中 EWMA/斜率/波动分类 | 缺**定时作业写入 `TrendAnalysis`**、缺异常检测阈值配置 |
...
3. `TopicVersion.policyJson` **结构化**：权重、decay、信源 tier 与 `scoreEntity` 对齐（**部分已有**：物化路径使用 `policy.weights` + `scoreEntity`；可再加强校验与 admin 表单）。

### Phase B — AI 编排

1. BullMQ DAG：`ranking.completed` → `trend` → `summary` → `credibility`。  
2. 多 `AiAnalysis.agent` 字段区分 Agent。  
3. Prompt 模板与**配额/审计**（生产必须）。
